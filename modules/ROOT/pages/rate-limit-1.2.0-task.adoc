...
=== Rate Limiting for Service Mesh

The Basic Rate Limiting Policy works in the same way as the regular policy, with the following limitations:

* Does not support sub-bucketing by an identifier (single bucket only)

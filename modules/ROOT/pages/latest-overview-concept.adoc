...
API Manager is a component of Anypoint Platform for managing APIs. The component leverages the runtime capabilities of API Gateway, which enforces policies, collects and tracks analytics data, manages proxies, provides industry standard encryption and authentications, and manages applications.
...
. Click *Save* to finish configuring the API. Since the API is unprotected, aou can now perform management tasks like applying policies, setting up SLA tiers, assigning permissions to the API environment, versioning, viewing API analytics, and more. 
+

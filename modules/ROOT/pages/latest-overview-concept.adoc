...
:page-notice-banner-message: Use the enhanced MuleSoft experience to secure and monitor APIs across providers, apply governance policies to external gateways, and manage LLM usage, costs, and routing with consistent governance across models. <a href="https://docs.mulesoft.com/general/exp-instances-add" target="_blank" rel="noopener">Learn More</a>.
...
It provides an interface to configure the runtime capabilities of Anypoint Omni Gateway, Anypoint Mule Gateway, Anypoint Service Mesh, and server instance types such as Agents, MCP servers, and LLM servers.
...
+
The _Agent and Tool Instances_ tab represents server instances that extend Anypoint Omni Gateway with additional capabilities such as Agents, MCP servers, and LLM servers. You can add, configure, and manage these server instances directly from this section.  
...
An API instance describes a configuration of an API that is deployed on one of the following runtimes: Omni Gateway, Mule Gateway, or Anypoint Service Mesh.  

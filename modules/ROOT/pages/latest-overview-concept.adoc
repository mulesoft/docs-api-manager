...
The following diagram outlines one relationship between the tools:

image::hello-world-flow-diagram.png[40%,40%]

. Build an RAML or OAS API specification in Design Center.
...
+
With the mocking service configured, test the mocked API with a tool such as `curl`, or test the mocked API in Exchange.
...
The endpoint must be actively tracked ("Active)") by Anypoint Platform in order for governance policies and SLA tiers to function.

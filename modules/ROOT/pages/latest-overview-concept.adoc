...
It provides an interface to configure the runtime capabilities of Anypoint Flex Gateway, Anypoint Mule Gateway, Anypoint Service Mesh, and server instance types such as Agents, MCP servers, and LLM servers.
...
+
The _Agent and Tool Instances_ tab represents server instances that extend Anypoint Flex Gateway with additional capabilities such as Agents, MCP servers, and LLM servers. You can add, configure, and manage these server instances directly from this section.  
...
An API instance describes a configuration of an API that is deployed on one of the following runtimes: Flex Gateway, Mule Gateway, or Anypoint Service Mesh.  
...
API Manager also manages *server instances* (Agent and Tool Instances) that extend Flex Gateway functionality through Agents, MCP servers, or LLM servers. These are managed separately from API instances.

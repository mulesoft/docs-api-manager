...
Anypoint API Manager (API Manager) is a component of Anypoint Platform that enables you to manage, govern, and secure APIs and server instances.
It provides an interface to configure the runtime capabilities of Omni Gateway, Anypoint Mule Gateway, and server instance types such as Agents, MCP servers, and LLM servers.
...
* xref:exchange::index.adoc[Exchange]: To store and publish API and server assets
...
+
The _Agent and Tool Instances_ tab represents server instances that extend Omni Gateway with additional capabilities such as Agents, MCP servers, and LLM servers. You can add, configure, and manage these server instances directly from this section.  
For details, see xref:create-instance-task-agent-tool.adoc[].
* *Automated Policies* 
+
...
* *Custom Policies*
+
_Custom policies_ are policies that anyone can develop and apply to their APIs, with the intention of extending existing functionality or defining new functionality.
...
. The *Environment information* button, available only to administrator users. Enables administrators to display a dialog with information about the current environment, such as environment credentials. Use environment credentials to configure Studio to sync with your environment.
...
* *Unregistered*: API Manager has no current tracking data for this API version. This can occur either because the endpoint has never communicated with the platform *or* because tracking data (for example, `lastActiveDate`) was purged after an extended period of inactivity (for example, ~30 days of no activity).

NOTE: Governance policies and SLA tiers require the endpoint to be in *Active* status to function. 

== API Manager Components, Concepts, and Features

=== API Instances

An API instance describes a configuration of an API that is deployed on one of the following runtimes: Omni Gateway or Mule Gateway.
API Manager manages API instances after they are created by using the *Add*, *Promote*, or *Import* options. API instances remain under management until they are deleted.  
...
NOTE: Multiple `autodiscovery` elements referencing the same API instance are not supported. Each Mule application can be linked to only one API instance in API Manager.

...
When creating or editing APIs in API Manager, you can use the xref:apikit::apikit-simulate[API Console] to view and simulate your API specification. This feature is available for RAML or OAS APIs, when you manage the API as an endpoint with proxy. The console flow is already included and enabled (by default) in the API specification when you download an out-of-the-box proxy for your Mule application.

Starting with Mule 4, you can enable access to the API Console and modify the API Console URL from API Manager. The API Console access from API Manager is disabled by default. For information about how to enable the API Console access fromm API Manager, see xref:manage-exchange-api-task.adoc[Creating an API Instance by Importing From Exchange].

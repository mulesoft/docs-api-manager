...
+
If "No Data" is displayed, automatic monitoring has not been enabled for the API. For details, see xref:monitoring::configure-monitoring-cloudhub.adoc#enable-automatic-monitoring-for-apps-that-you-plan-to-deploy-to-cloudhub[Configure and Enable Anypoint Monitoring for Mule Apps (CloudHub)]

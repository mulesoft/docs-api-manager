...
== Hello World
The following procedure describes how to import an Exchange API asset into API Manager. 

. Once logged in to API Manager, click the environment control and switch to an environment containing your API.
. Click *Manage API*.
. Select *Manage API from Exchange*.
. In the *Manage API from Exchange* view, type the name of your API next to the *API Name* field.

...
API Manager is a component of Anypoint Platform for managing APIs. The component leverages the runtime capabilities of API Gateway, which enforces policies, collects and tracks analytics data, manages proxies, provides industry standard encryption and authentications, and manages client applications.
...
Mule Applications are also referred to as:
...
=== API Alerts
An API alert (different from a Runtime Manager alert) is an alarm that flags one of the following issues:
...
=== API Assets
API assets, published by Exchange, refer to any of the following components:

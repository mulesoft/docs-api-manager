...
An API group is a collection of API instances that act as a single unit, so that applications can access the APIs as a group using one client ID. API groups are created in API Manager and published to Exchange.

For details, see xref:api-groups-landing-page.adoc[API Groups].

=== API Instances

An API instance belongs to one environment, but you can have different API instances of an API in different environments.

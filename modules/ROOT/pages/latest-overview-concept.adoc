...
<2> The left navigation menu, with several available options:
** *API Groups*
+
...
<4> The *Promote from environment* button 
+
Promote an API from one environment to the current environment. 
+
For details, see xref:promote-api-task.adoc[Promoting an API Instance to Another Environment]. 
<5> The managed API search field
<6> The *Environment information* button 
+
Display a dialog with information about the current environment, such as environment credentials and business group credentials.
<7> The API version 
+
...
<8> The registration status of the API 
+
...
<9> The number of client applications that have contracted with the API
+
For details, see xref:api-contracts-landing-page.adoc[Client Applications and Contracts].

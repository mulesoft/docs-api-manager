...
+
_Agent and Tool Instances_ represent server instances that extend Anypoint Flex Gateway with additional capabilities such as Agents, MCP servers, and LLM servers. You can add, configure, and manage these server instances directly from this section.  
...
API instances are managed by API Manager after they are created by using the add, promote, or import options. API instances remain under management until they are deleted.  

...
The following flow diagram outlines a typical relationship between the tools:

image::hello-world-flow-diagram.png[40%,40%]

. Build your RAML or OAS API specification in API Designer.
. Publish the API specification from API Designer to Exchange.
. In API Manager, via *Manage API*, add the API that was published from Exchange.
. In API Manager, add a rate-limiting SLA tier to the newly added API. 
. Open Exchange from API Manager (view the added API in Exchange).
. In Exchange, create a client application for the API via the *Request Access* UI. API Manager is automatically notified that a client application is requesting access.
. In Exchange, test the API against the Mocking Service. 

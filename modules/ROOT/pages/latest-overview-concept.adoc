...
<3> The *Add API* button. Enables you to add new API instance, to promote an API from any environment to the current environment, or to import a configuration ZIP file exported from API Manager. For details, see xref:getting-started-proxy.adoc[Getting Started with Managing an API].
...
<5> Search. Enables you to search for managed APIs via the API search field. Note that searches are case-insensitive. Filter search results by selecting *Active*.
...
<8> The percentage of API requests that resulted in errors (in the previous 24 hours).
+
...
<9> The total number of API requests (in the previous 24 hours).
+
If "No Data" is displayed, automatic monitoring has not been enabled for the API. For details, see xref:monitoring::quick-start.adoc[Monitoring Quick Start].

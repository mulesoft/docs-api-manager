...
Additionally, download and install the latest versio of Studio. For more information, refer to xref:studio::to-download-and-install-studio.adoc[Downloading and Installing Anypoint Studio].
...
==== Step 3.5: Test the API in Studio
. Open a command line client such as Terminal.
. To make a request against the API, type the following `curl` command into the command line client, followed by RETURN:
+
----
...
----
. Verify that the *Rate limiting - SLA based* policy is active. Since the policy was previously configured to only allow three requests per minute, repeating the above `curl` command three more times will result in the following response:
+
----
{

...
* The first item shows the current environment, which, when clicked navigates between environments. Anypoint Platform enables you to create and manage separate deployment environments for APIs and applications. 
+
For details, see xref:environments-concept.adoc[Reviewing Environment Concepts].
* The second item shows the left navigation menu, with *API Administration* currently selected. Several additional options are presented:
** *API Groups*. An API Group is an API asset that enables organizations to publish a group of API instances as a single unit. 
+
For details, see xref:api-groups-landing-page.adoc[API Groups].
** *Automated Policies*. With policy automation, API owners can quickly design, build, and deploy secure and consistent APIs. 
+
For details, see xref:automated-policies-landing-page.adoc[Automated Policies].
** *Client Applications*. Applications are packages that integrate connectors, modules, and components. 
+
For details, see xref:view-api-contracts.adoc[View API Instance Contracts].
** *Custom Policies*. Custom Policies are policies that anyone can develop and apply to their APIs, with the intention of extending existing functionality or defining new ones. 
+
For details, see xref:custom-policy-getting-started.adoc[Getting started with Custom Policies development].
** *Analytics*. API Analytics can provide insight into how your APIs are being used and how they are performing. 
+
For details, see xref:viewing-api-analytics.adoc[Reviewing Analytics Usage].
* Clicking the third item adds an API to API Manager. 
+
...
* The fourth item enables you to promote an API from one environment to the current environment. 
+
For details, see xref:promote-api-task.adoc[Promoting an API Instance to Another Environment]. 
* The fifth item enables you to search the API list for a managed API.
* Clicking the sixth item displays a dialog with information about the current environment, such as environment credentials and business group credentials.
* The seventh item shows the API version. 
+
Clicking the version number navigates you to the API settings view, where you can manage the following:
+
** *Alerts*. For details, see xref:using-api-alerts.adoc[Reviewing Alerts Concepts].
** *Contracts*. For details, see xref:api-contracts-landing-page.adoc[Client Applications and Contracts].
** *Policies*. For details, see xref:api-proxy-landing-page.adoc[API Proxy].
** *SLA Tiers*. For details, see xref:defining-sla-tiers.adoc[Reviewing SLA Tiers Concepts].
* The eighth item indicates the registration status of the API. Possible values are *Active*, *Inactive*, and *Unregistered*. 
+
...
* The ninth item indicates the number of client applications that have contracted with the API.
+

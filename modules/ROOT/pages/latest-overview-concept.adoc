...
An API instance describes a configuration of an API that is deployed on one of the following three runtimes: Flex Gateway, Mule Gateway, or Service Mesh.

API instances are managed by Anypoint Platform after they are created using add, promote, or import options in API Manager or other Anypoint Platform components. API instances remain under Anypoint Platform management until they are deleted.

...
To configure the runtime through the agent to use encryption the argument `encryption-key` with the desired key as value. The agent then encrypts the client ID and Secret.
...
The gateway encryption tool provides a way to inspect the policy files that Mule runtime 3.9.3 and later encrypts, provides a way to encrypt offline policies, and pass encrypted client credentials to the runtime.
...
When using the `decrypt` option, it is possible that your terminal might interpret some of the characters of the encrypted value. To avoid this scenario, wrap the encrypted value in simple comas:
...
When running in clusters, offline policies remain encrypted in the node in which the policy was originally deployed, but it will propagate unencrypted to the other nodes.

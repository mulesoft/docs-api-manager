...
You can specify service-level agreement (SLA) tiers to API Groups instances to limit the number of requests an application (user) can make to the APIs grouped in an API Group. SLAs enable you to protect your API Group instances and help manage network traffic to your APIs.
...
The default limit is set by the Group Administrator and applies to all API instances. If you add a new API instance to an API group instance, the default limit SLA configured at the group instance level applies, unless an individual API SLA tier set by the Group Administrator. 

== Individual API SLAs

As a Group Administrator, you can define individual limits for API instances within an API Group instance.  If you have configured a default SLA limit at the API Group instance level and an individual SLA limit at the API instance level, the individual SLA takes precedence over the default limit. 

...
You xref:setting-up-an-api-proxy.adoc[deploy an API or proxy to a Mule 3.9.0] runtime, or to an earlier API Gateway Runtime if you are running Mule versions earlier than 3.8.
...
== On-Premises Using Runtime Manager

This procedure works with API Gateway 2.0 and above.

. Register your server in Runtime Manager using these simplified instructions or the xref:runtime-manager::managing-servers.adoc#add-a-server[full instructions].
...
+
image::conf-api.png["API configuration page showing Configure endpoint option under API Status"]
+
. In the endpoint configuration screen, fill in the required information for the API you would like to manage.
+
image::conf-endpoint-props.png["Endpoint configuration dialog showing implementation URI and deployment options"]
+
. Click **Save & deploy**. You see the status of your gateway is running.
+
image::deploy-proxy.png["API Gateway deployment interface showing runtime selection and deployment status"]
+
...
+
image::no-server.png["Runtime Manager interface showing no registered API Gateway servers available"]
+
The screen gives you the option to add a new API Gateway.
+
. Select your gateway, then click *Deploy proxy*.
+
image::deploying.png["Deployment progress interface showing proxy deployment status and completion"]
+
...
+
image::deploy-ch.png["CloudHub deployment configuration showing proxy settings and port configuration"]
+
...
+
image::deploy-ch-2.png["CloudHub proxy deployment screen"]

. In the deploy menu, pick an app name, environment, and API Gateway version, or accept the defaults.
+
Under the API Status section, a new link appears labeled *Manage CloudHub proxy*. This links you to the actual application on Runtime Manager for further management, such as changing the worker type and advanced settings.
+
...
+
image::add-cloudhub-app.jpg["Runtime Manager Add Application screen for creating a new CloudHub application"]
+
...
. Runtime Manager automatically moves to the *Logs* view where you can track the status of the deployment. Watch for this message:
+
image::proxych-started.png["CloudHub proxy deployment status showing successful startup"]
+
...
If you plan to expose your API through SSL, then there are a couple of xref:mule-runtime::build-an-https-service.adoc[additional steps] you need to take.

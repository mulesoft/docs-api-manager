...
* Any one of the following policies available in federated organizations, configured with specific OAuth 2.0 providers:
...
None of these token enforcement policies allow access to OAuth 2.0 protected resources if credentials from non-Mule client applications that are created in Anypoint Platform are used.
...
To apply OAuth policies, you must:
...
* Configure your Anypoint Platform organization as a federated organization using either OpenAM, OpenID Connect, or PingFederate. Alternatively, you have a xref:mule-oauth-provider-landing-page.adoc[Mule OAuth 2.0 Provider] configured and running.

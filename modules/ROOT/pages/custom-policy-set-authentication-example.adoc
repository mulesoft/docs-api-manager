...
Because every user ID is mapped to the team's client ID, the client ID is propagated with every transaction made by any user ID. When the client ID is injected into the authentication data of the Mule event, Anypoint Platform generates analytics data for the client application. Team owners uses this application analytics to review the team transactions in the application.
...
ProTeams uses the Set Authentication custom policy example as a starting point and then adds their own mapping logic to it.

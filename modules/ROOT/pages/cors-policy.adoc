...
Origin is a header that indicates the request from where a fetch originated. The origin header includes only the server name (no path information). The header is sent with CORS and POST requests. 

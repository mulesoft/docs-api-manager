...
Anypoint Platform alerts you about unusual request behavior, performance issues, actions related to client application contracts, and runtime events. The platform supports three types of alerts: API alerts, contracts alerts, and runtime alerts. API alerts and contracts alerts are managed by API Manager. Runtime alerts are managed by Runtime Manager.

== API Alerts
 
In API Manager, you add and configure API alerts that flag unusual behavior, such as if the response time of an API is greater than 60 seconds, or if the number of API requests in 30 seconds is greater than 1000. See xref:add-api-alert-task.adoc[Adding an API Alert].
...
+
Time interval values are in the range 1 - 999999999. To prevent data burst problems, the intervals are implemented as sliding windows instead of absolute minute boundaries. See https://www.techopedia.com/definition/869/sliding-window[Sliding Windows].

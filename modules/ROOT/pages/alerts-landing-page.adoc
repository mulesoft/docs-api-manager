...
+
Time interval values are in the range 1 - 999999999. The intervals are implemented as sliding windows instead of absolute minute boundaries, to prevent data burst problems. See https://www.techopedia.com/definition/869/sliding-window[Sliding Windows].
...
API Manager triggers API alerts only when states change. However the first occurrence of a state change may be undetected due to newly created alerts not having defined states.

When configuring an API alert, you select Business Group users who receive email notifications about the alert. The notification includes information about events that trigger the alert as specified by the level of severity. Users receive two emails:
...
The following lists actions that trigger contracts alerts, as well as the email notification recipients:

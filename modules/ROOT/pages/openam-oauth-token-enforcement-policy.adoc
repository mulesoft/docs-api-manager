...
[width="100%", cols="5,15"]
|===
>s| Policy Name | OpenAM Access Token Enforcement
...
The OpenAM Access Token Enforcement policy restricts access to a protected resource, by only allowing HTTP requests if the token provided in such request is a valid one and, optionally, the required OAuth scopes are fulfilled.
...
To control token cached time (for example, token cache time before retrying validation against the OAuth Provider), you may specify the following property:
...
To enable the OpenAM Access Token Enforcement policy to use the Gateway proxy settings, specify the following property:

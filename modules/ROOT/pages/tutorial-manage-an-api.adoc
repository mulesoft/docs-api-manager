...
. Click *Actions*.
. Click *Change API Specification*. 
...
[[register-deploy]]
== Register and Deploy
After applying the policy, the following steps are required in order for the policy to take effect:
...
. Test using a tool such as Advanced REST Client, making sure to populate the request's `client_id` and `client_secret` header fields with the client credentials obtained earlier.

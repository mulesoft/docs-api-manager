...
The operations was introduced to allow DataWeave expressions when the policy configuration comes form a property of type `keyvalues` with multiple values.
...
With the configuration shown in the previous example we can use handlebars to expand the headers to be added in the policy, as shown below.
[source,xml,linenums]
----
...
<http-transform:add-request-headers-list>

...
API Gateway Runtime 2.x has a domain named *api-gateway*. In Mule 3.8.x, due to the unification of API Gateway Runtime with xref:release-notes::mule-runtime/mule-3.8.0-release-notes.adoc[Mule Runtime 3.8.0] and several usability issues, the api-gateway domain has been removed. You can install the api-gateway domain by xref:release-notes::api-gateway/api-gateway-runtime-to-mule-3.8.0-migration-guide.adoc#migration-procedure-other-users[running a script] provided in the Mule 3.8.x distribution. Studio 6.x and CloudHub seamlessly support the domain you install using the provided script.
...
+
. In *Proxy settings*, select *Reference user domain (only for API Gateway >= 3.8)* only if you have created a domain by xref:release-notes::api-gateway/api-gateway-runtime-to-mule-3.8.0-migration-guide.adoc[running the script] provided in Mule 3.8.0 or copying the domain from a previous API Gateway runtime version.
...
If you have xref:release-notes::api-gateway/api-gateway-runtime-to-mule-3.8.0-migration-guide.adoc[created a domain], you can use your own listener-config in your app or proxy under the following conditions:

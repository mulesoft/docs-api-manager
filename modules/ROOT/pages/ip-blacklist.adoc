...
The IP Blacklist policy controls access to a configured API endpoint from a range of IP addresses. You can blacklist access based on:

* The IP address of the request
* The origin IP address of the header type, such as https://en.wikipedia.org/wiki/X-Forwarded-For[x-Forwarded-For] header or HTTP header.
* Any other origin
...
* Blacklist access based on the origin IP address that is determined by resolving a specific DataWeave or MEL expression, such as the `x-Forwarded-For` header or the `HTTP` header.
...
|Blacklist
|The list of IP addresses that must be denies access to the API. You may define one IP or an IP range at a time, as many times required. For more information, see <<specifying_ip_address,Specifying IP Addresses to Blacklist in the Policy>>.

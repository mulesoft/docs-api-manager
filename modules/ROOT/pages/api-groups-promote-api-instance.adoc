...
The API instances of the promoted API Group instance may differ from the original group instance, but the definition (related assets) remains the same.
...
* Verify that the APIs in the API Group instance that you want to promote are already present in the destination environment. For example, if the API Group Instance that you want to promote contains the Facebook API, the destination environment must already have the Facebook API in that environment type.

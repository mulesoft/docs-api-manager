...
. Search for your organization's ID to use in the following steps.
...
. Go to your `pom.xml`
. Replace the {groupId} with the organization's ID from the previous state.
[source,XML,linenums]
+
--
<groupId>{orgId}</groupId>
--
. Add the following properties replacing the {orgId} for the organization's id from the previous setp:
...
--
. Finally to deploy the policy use `maven clean deploy`.
+
[NOTE]
--

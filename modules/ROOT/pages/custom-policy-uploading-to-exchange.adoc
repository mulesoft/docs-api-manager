...
. Search in Anypoint Platform your organization's ID to use in the following steps.
...
* Mule Maven Plugin: It will validate that all necessary files are present and it is require to correctly package policy.
* Maven Deploy Plugin: It will communicate with the exchange repository and upload the policy jar and yaml.
+
[source,XML,linenums]
--
<build>
...
--
. Finally, deploy the policy using `maven clean deploy`.
+
[NOTE]
--

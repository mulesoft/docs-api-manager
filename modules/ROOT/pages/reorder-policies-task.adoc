...
. In Reorder Applied Policies, use the up and down arrows to rearrange the order of policies. For example, if you apply rate limiting, IP allowlist, and XML threat protection, you can reorder only the IP allowlist and XML threat protection policies because rate limiting takes precedence over the other policies.
. Click Apply order.

...
image::oauth-policy1.png["OAuth 2.0 authentication flow diagram showing seven steps of token exchange and validation",height=366,width=640]
...
. The validated token is allowlisted and kept on record until expiration. Any further requests that contain this token are not validated against the OAuth provider.

...
// what does this sentence mean--regardless..?--Eva


//Apart from analytics, what problem does it solve for the users.
//What information is stored or retrieved.
// How does the policy work?
//Can we add a real-world use case here? Like create a real life example where a user is having a problem and how this helps them ease their lives.
...
==== With WWW Authenticate HTTP header
When an HTTP request is performed against a protected resource for which the Client ID Enforcement policy is applied, and client application credentials are invalid or unauthorized, the HTTP response includes a WWW Authenticate header with the following values:

...
For Service Mesh, there is a variety of this policy, named Basic Client ID Enforcement Policy. Although the functionality is similar to the non-basic kind, there are differences in the policy configuration:

[%header%autowidth.spread,cols="a,a,a"]
|===
| Element | Description | Required
| Credentials origin 
| Specifies from where in the request the values will be extracted:

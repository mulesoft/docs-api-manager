...
To improve performance, once a token is validated, it is cached by the system by default, until the token expires. In certain cases, you might reduce or even disable the caching mechanism, for example to take into account revoked token scenarios.

After the system validates a token, by default the token is cached until expiration to improving the performance. In certain cases, such as revoked token scenarios, you might reduce or even disable caching.

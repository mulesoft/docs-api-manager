...
[width="100%", cols="5,15"]
|===
>s| Policy Name | OpenID Connect Access Token Enforcement
...
You can enable the OpenID Connect Access Token Enforcement policy to use the gateway proxy settings by specify the following property when starting the Mule runtime engine: 
...
*Can I configure the Rate Limiting - SLA-based policy after I configure the OpenID Connect Access Token Enforcement policy?*

Yes, you can. Apply the Rate Limiting - SLA-based policy after the OpenID Connect Access Token Enforcement policy and provide a `Client ID Expression` value using the following DataWeave 2.0 expression: 

...
The easiest way to bring all your required files is by using the maven archetype. This archetype creates all the necessary files for you.
You also need to use Maven package your custom policy into a deployable JAR.
...
Once Maven is configured, follow the following steps:
...
Replacing:
+
* `${orgId}` with Anypoint Platform Organization Id where the policy will be uploaded to.
...
Those four files are the basic ones needed for having a working policy. A brief comment on each:

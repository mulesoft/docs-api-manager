...
Policies can be default, custom, or automated. Each type of policy has its own scope, management, and usability. You can uniformly apply a policy to all APIs or you can apply policies to select API based on a criteria, which is known as resource-level policies.

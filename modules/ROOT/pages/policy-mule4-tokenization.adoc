...
[NOTE]
--
This policy is designed to run only in a Runtime Fabric environment.
--

== Tokenization

Tokenization is the process of transforming a value or piece of information (presumably sensitive data) into another one (i.e., the token), that can be mapped back onto the original value. +

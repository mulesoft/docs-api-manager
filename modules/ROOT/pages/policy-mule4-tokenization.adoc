...
>s| Summary      | Transforms sensitive data into non-sensitive equivalent, named token. The data is displayed identical to the absolute value in format, but its actual value is not revealed at first sight.
>s| Category | Security
...
This tokenized information replaces the original value with the token, thus minimizing the risk of matching it to the absolute value, in case a third party accesses the data. For example, this policy may be used to tokenize credit card numbers and map them to completely different ones from the originals.
...
* The *Manage Policies* permission.
* The *Manage Tokenization Services* permission to xref:anypoint-security::tokenization-example.adoc[configure] and see the tokenization service. 

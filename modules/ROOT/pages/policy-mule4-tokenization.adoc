...
>s| Summary      | Transforms sensitive data into a nonsensitive equivalent, named token. The data is displayed in the same format as the original value, but its actual value is not revealed at first sight. This policy is designed to work only for Anypoint Runtime Fabric deployments.
>s| Category | Security
...
!===
.2+>.^! 400 ! A token is invalid, or the tokenization failed.
! The provided expression is invalid or returns no result to tokenize. +
The status code to be returned can be configured.
! 502 ! Any problem regarding the connection with the tokenization service, such as the service is unavailable, or the service is not configured.
!===

...
a xref:rate-limiting-sla-policy.adoc[Rate-Limiting SLA policy]. In this use case, the first policy
validates a token provided in the request. If the token is valid, the client ID is retrieved from the provider and saved in the Event authentication object. This authentication data is then used by the Rate Limiting policy to verify that 
...
This operation sets new authentication information to the Mule event. The operation overwrites any existing authentication data.

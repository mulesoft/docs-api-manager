...
API Manager is closely integrated with xref:api-governance::index.adoc[Anypoint API Governance]. Use xref:api-governance::index.adoc[Anypoint API Governance] to govern API instances with governance rulesets. Applying a governance ruleset to your API ensures that every instance of that API conforms to standards, such as which policies are applied, or whether a TLS context is required.

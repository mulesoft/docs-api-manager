...
For example, the following parameter configuration removes the `Access-Control-Allow-Credentials` header from a message but also any other starting with `Access-Control` (like `Access-Control-Allow-Origin`). It also removes the host header:

...
include::partial$task-configure-proxy.adoc[tags=intro;first-steps;target-type-heading;target-type;mid-steps-heading;mid-steps;asset-type-options;mid-steps2;raml-oas-version,leveloffset=+2]
...
You should now see the *TLS Context for Inbound Traffic* field.

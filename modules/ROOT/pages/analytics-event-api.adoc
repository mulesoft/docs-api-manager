...
If you are interested in data sync for events with an external system, refer to xref:analytics-event-forward.adoc[Analytics Event Forwarding] for more information.
...
The previous example gets the analytics for organizations without environments. In the following snippet, there’s another example of querying the Events API, but this time, for organizations with environments.
...
So, to understand what we’ve just done, we want all the analytics for the past 30 days from the API with API ID <API_ID>; and we want the response to be in CSV format.

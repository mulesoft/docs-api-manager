...
On the portal for the API, you request access to an API, and you receive credentials to access the JSONPlaceholder API from your application. You base64-encode the credentials that the application receives to access the API. Using the encoded credentials, you get an access token from the provider--steps 1-2 of the xref:aes-oauth-faq.adoc[OAuth dance]. Use the token to call the JSONPlaceholder service--steps 3-6 of the OAuth dance. If the token isn't properly validated, a `403` error message is returned to the client application; otherwise, the API returns results, the list of users--step 7 of the OAuth dance.

. Create and deploy the xref:designing-your-api.adoc#creating-a-raml-1-0-based-api[JSONPlaceholder service API] proxy using API Manager.
+
You can
...
+
image::building-an-external-oauth-2_0-provider-application-8353f.png["OAuth 2.0 provider configuration showing access token validation endpoint URL field",height=393,width=417]
+
. xref:tutorial-create-an-api-portal.adoc[Create an API portal] for the API.
. xref:browsing-and-accessing-apis.adoc#accessing-api-portals[Request access] for a client application to the JSONplaceholder service API.
+
...
+
image::building-an-external-oauth-2_0-provider-application-1da90.png["Postman interface showing successful API response with user data",height=393,width=417]

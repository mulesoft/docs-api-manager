...
Mule OAuth 2.0 Client Store has been introduced in Mule 4.2.0, allowing the OAuth Provider to use Anypoint Client Applications for token generation.

The Client Store requires connectivity to Anypoint Platform, therefore the Mule Runtime hosting the OAuth Provider must must be licensed for running an API Gateway and contain proper startup credentials.
...
+
Defines whether the caching mechanism should persist locally. The default value is "true" for when the property is not set or cannot be parsed. Cache persistence allows client authorization when there is no connection to Anypoint Platform.

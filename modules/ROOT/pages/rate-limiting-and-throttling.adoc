...
image::throttling-rejected-request.png["A queued request is rejected after being reprocessed"]
...
image::throttling-accepted-request.png["A queued request is accepted after being reprocessed"]
...
image::quota-policy-formula.png["Formula for configuring the quota policy"]

where Omega image:omega.png[""] is a small number, below the backend maximum capacity.

...
Some instances in *Agent and Tool Instances* might have been created by supplying YAML in an agent network project. For instances created with YAML, the YAML is treated as the source of truth. If you edit the instance in API Manager (for example, to add policies), the changes are reverted to the YAML when the instance is redeployed from the agent network project.

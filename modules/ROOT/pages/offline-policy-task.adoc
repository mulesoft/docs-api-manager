...
An offline policy is applied directly to the runtime and not through API Manager. This procedure describes how to apply an offline policy to an API. When you create a file in the offline-policies directory, Mule runtime engine (Mule) detects the presence of the file and starts applying the policy. 

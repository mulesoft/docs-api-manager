...
The LDAP Security Manager policy establishes the configuration details for an Open LDAP or Active Directory LDAP that you have set up for your enterprise. To enforce the Basic Authentication - LDAP policy policy, you also need to create and apply an Basic Authentication: Simple policy, which prompts the agent to enforce credentialed access using the configuration established in your LDAP policy.
//Is this correct Mariano?

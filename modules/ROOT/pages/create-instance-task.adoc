...
+
include::partial$task-configure-proxy.adoc[tags=target-type;mid-steps;asset-type-options,leveloffset=+2]
9. Configure the Endpoint and other settings:
+
...
+
* Update the xref:building-https-proxy.adoc#configure_options[optional fields] if needed. For further details on these optional fields, see xref:configure-api-task.adoc[Configure an API Endpoint].

10. Click **Next**.
11. Review your selections. You can edit each section if necessary.
12. If you are ready to deploy, click **Save & Deploy**, otherwise you can select **Save**, to save the API Instance

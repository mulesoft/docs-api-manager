...
+
image::autodis-pairing.png["API autodiscovery pairing interface showing connection settings between API and runtime"]
+
...
+
image::autodis-cerate-global2.png["Global element configuration interface for API autodiscovery settings"]
+
...
+
Do not change the version name later in API Portal, as explained in xref:api-auto-discovery.adoc#best-practices["Best Practices"].
...
+
image::auto-disc-conf2.png["Autodiscovery configuration interface with runtime settings and connection options",height=422,width=439]
+
* Click OK.
+
...
+
image::autodisc-discovered2.png["Autodiscovery results showing detected APIs and their status",height=164,width=561]
+

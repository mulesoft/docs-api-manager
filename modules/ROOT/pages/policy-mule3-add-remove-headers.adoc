= Header Injection and Removal Policy
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:keywords: header, policy

API Manager supports policies for injecting or removing HTTP headers from a request or response. The policies take effect before sending the request or response.

== Add Request and Add Response Headers

The Add Request Header policy injects one or more headers into a request from the client to the backend service, resulting in a request with old values and new ones. The Add Response Header policy injects one or more headers into a response from the backend service to the client. 

For example, in the a request with header "foo=value1", if we add the header "foo=value2" with the policy, the result will be a request with header "foo=value1, value2"

== Remove Request and Remove Response Headers

The Remove Request Header policy prevents receipt of one or more specified headers sent from the client to the backend service. The Remove Response Header policy prevents inclusion of one or more specified headers in a response from the backend service to the client.
...
=== Applying the Header Injection Policies

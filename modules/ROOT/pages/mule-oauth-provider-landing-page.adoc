...
+
This feature is availble in Mule 4.2.0 and later.
...
You can customize the default values of the cache mechanism in Mule 4.2.0 and later.

The object store has two configurable properties:
[%header%autowidth.spread,cols="a,a"]
|===
| * `anypoint.oauth_provider.cache_persistence_enabled`
| Defines whether the caching mechanism must persist locally. The default value is `true` if the property is not set or cannot be parsed. Cache persistence allows client authorization without connection to Anypoint Platform.
| * `anypoint.oauth_provider.cache_ttl_days`
| Represents how many days each client application must be cached by the OAuth Server. You can specify any positive floating-point number for this value. The default value is `30`, representing 30 days.
|===

...
An API proxy is an intermediary layer between the client applications and the backend APIs. 

The API proxy intercepts incoming API requests, and provides authentication, rate limiting, and request/response transformation features.

When an external consumer application sends a request to an API, the request is first intercepted by the API proxy endpoint. This proxy applies all policies for the requested API, and uses the policies to determine if, and how, to forward the request to the API implementation.

If all the policy validations and actions are successful, the proxy forwards the request to your backend API. 

The API proxy then intercepts the response from your backend, and applies any policies relevant to the response, and then returns the API response to the consumer application. Conversely, if a policy fails any of the validations, the API proxy returns an error response (appropriate to the failed policy) and the request does not reach your backend. 

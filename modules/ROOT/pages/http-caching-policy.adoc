...
//Note that this step refers to the actors as "the client" and "the API" rather than user request, HTTP request, endpoint, or backend. Using the same terms for the same things throughout a narrative eases comprehension.
...
// (@lorena--the request reaches the policy or the response?)
. Caching hits the object store again and attempts to save the <<key,key>> of the request for later reuse.
//@Lorena--Do we mean that the policy hit the object store again?
. The response continues through the remaining policies in the policy chain.
. The response is returned to the client.
//we seem too be interchangeably talking about the response and request. It's a bit confusing.
//Didn't the request travel to the backend and then travel back in the opposite direction as a response? So the request goes through policies A to D, for example, and the response through policies D to A, "returning to the first" in the chain rather than reaching the last?
...
//When you upgrade to a Mule version?--Lorena
...
//if they are syntax, format them in monospace and exactly mimic the onscreen capitalization.

...
*Note:* When upgrading a runtime version and the caching policy is configured to use persistent store, the caching policy will do a best effort to maintain the entries stored by the previous runtime.
In the worst case scenario, the entries in the cache will be invalidated and the cache will be populated anew as new requests arrive. This is automatic and transparent to the user.

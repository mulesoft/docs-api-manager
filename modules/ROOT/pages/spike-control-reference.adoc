...
// The Spike Control policy uses a Sliding Window Algorithm to focus exclusively on reducing spikes in traffic and protecting the Mule Runtime Engine (Mule) instance. The window in the algorithm adjusts as time progresses, retaining only the processed information that fits within the window time span. 
...
Queuing a request requires retaining a thread and an HTTP connection. The queuing limit mechanism protects the API to ensure the API does not fail in case of an attack.
...
// But is it really 1 second from "request #1" if "the first two requests are accepted and the quota is 0"? -- Nahuel?
. Request #3 arrives. 
+
Because no quota is available, the request is queued for 499 milliseconds.
//How can we say no quota is available when we just said that the quota increased to 1?--Nahuel

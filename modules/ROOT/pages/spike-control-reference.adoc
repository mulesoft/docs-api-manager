...
[%autowidth.spread,cols="a,a"]
!===
.3+>.^! 429 ! The number of requests by HTTP APIs exceeded the configured limit! Request rejected after specified number of reattempts.
.3+>.^! 400 ! The number of requests by WSDL APIs that use SOAP v1.2 exceeded the configured limit! Request rejected after specified number of reattempts.
.3+>.^! 500 ! The number of requests by WSDL APIs that use SOAP v1.1 exceeded the configured limit! Request rejected after specified number of reattempts.
!===
...
To enforce request quotas, see the xref:rate-limiting-sla-based-policy.adoc[Rate-Limiting SLA policy] and xref:rate-limiting.adoc[Rate Limiting policy].
...
The algorithm uses a sliding window by monitoring the transactions for the amount of time for which you configure in the policy.

*What happens when the quota is exhausted?*

When the request quota is exhausted, the Spike Control policy queues the request and retries as configured. The request is rejected if no quota is remaining after several retries. Quota for other requests is only available after the first request has elapsed the time in millisecconds as configured in the policy. Requests are processed in cycles using the configured time limit in this way.
...
* X-Ratelimit-Reset: The remaining time, in milliseconds, until the oldest request ages enough to fall outside of the sliding window. If quota is still available, then this header is set to 0 (zero), as the algorithm can still serve new requests.

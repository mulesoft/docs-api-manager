...
+
If quota is still available, then this header is set to 0 (zero), becuase the algorithm can still serve new requests.

Expose these headers only if the API is used within the organization. For public APIs, this data can be used maliciously to invoke spikes in traffic that leaves your API without any quota left.

...
The Rate Limiting - SLA-based policy enables you to control incoming traffic to an API by limiting the number of requests that the API can receive within a given timespan. When the limit is reached before the time expires, the policy rejects all requests, thereby avoiding any additional load on the backend API.

To apply the Rate Limiting - SLA-based policy to an API, you must first create a contract between the API and a registered client application. The number of requests that an API can receive within a given time is defined in the xref:manage-client-apps-latest-task.adoc[contracts] section in API Manager. 
...
The Rate Limiting - SLA-based policy monitors the number of requests made in the current window (the available quota), allowing the requests to reach the backend only if the available quota is greater than zero. 
...
To understand how the Rate Limiting - SLA-based policy works, consider an example in which the configuration of an SLA of 3 requests every 10 seconds for the client with ID “ID#1” allows or restricts the request, based on the quota available in that window:
...
An API might have several contracts, each with its own SLA. The Rate Limiting - SLA-based policy independently monitors the quota (and windows) available for each client by creating one rate-limiting algorithm per contract. With the first request to the API, algorithms are created using the _lazy creation_ strategy.
...
image:rate-limiting-clusterizable.png[Rate-Limiting SLA Clusterizable Configuration,75%,85%]
...
If you use only one client application as an authorization mechanism in your server, the same recommendations as in the Rate Limiting policy apply to your use case. You might have several Mule instances and you might map different client applications to different servers in the load balancer.
...
Mule clusters count the SLA quota for each client throughout the cluster and thus are a best fit for this use case. The Rate Limiting - SLA-based policy is designed to work under both balanced and unbalanced workloads. Because the backend does not receive any extra requests, its maximum capacity is not exceeded.
...
Therefore, you must set up window sizes greater than one minute in Rate Limiting and Rate Limiting - SLA-based policy configurations only for a cluster scenario.

=== Choosing Persistence for Your Rate Limiting - SLA-based Policy

You can configure your Rate Limiting - SLA-based policy to use windows that persist as long as days, months, and years. For example, suppose you want to enable your user to consume 1 million requests per year, but you cannot ensure that the node will be up the entire period or will not need maintenance, which might result in restarting Mule.
...
The Rate Limiting - SLA-based policy works in the same way for Anypoint Service Mesh (non-Mule applications), but with the following differences:
...
When the request quota is exhausted, the Rate Limiting - SLA-based policy rejects the request. When the time window closes, the request quota is reset and a new window of the same fixed size starts.

...
. Click Add.
+
image::gw-add-custom-pol.png["Custom policy configuration interface showing header modification options"]
+
The policy, add request header policy in this example, appears on the Custom Policies page.
+
image::gw-custom-pol-list.png["List of configured custom policies with header management settings"]
...
+
. Click the plus icon image:gw-apply-pol-plus-icon.png[""].

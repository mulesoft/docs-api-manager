...
+
The API Group Administrator permission is set at the business group level. Therefore, an API Group Administrator can create, update, delete all groups in the business group. The permissions also gives `View` access to all API instances of the business group to be able to add API instances to the API Group.

// Need to be very explicit how this permission is responsible for which API instances are available for adding in an API Group.
// How this permission allows you to manage SLAs, contracts and such, and other APIM permission won't let you manage group level activities
...
//what does environment type mean here--fede

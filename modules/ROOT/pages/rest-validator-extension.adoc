...
. <<add-namespacee-and-schema,Add the namespace and schema to your Mule artifact>>
...
== Configuring REST Validator Extension

To configure and use the REST Validator, add the following snippet in your Mule artifact:

`<rest-validator:config name="validatorConfig" api="${specFile}" />`

This snippet contains the required attributes for configuration. You can additionally specify the optional attributes to customize the extension The following example shows how to use attributes deserializing strategies:
[source,XML,linenums]
----
...
| `api` | If you are using a version of the REST validator later than 2.0.0, you must use the `raml` attribute. Location of the spec file against which the requests are to be validated. | No
...
* RAML: The APIKit router uses theRAML parser to read the API and validate incoming requests. | No
...
== Use the REST Validator Operations

After you configure the REST validator, you can start using the validator operations:
...
Validates request against the specified RAML or OAS spec specified in the configuration
...
If the request does not violate the defined schema, thus the validation ends successfully, the result of the operation will leave untouched the HTTP Request Attributes. If, on the other hand, default values were added as a result of the schema validation, the result will be the original HTTP Request Attributes plus the new ones.

==== Error Types

The ‘validate-request’ operation may raise different errors if a problem occurs when retrieving a file or if the validation does not end successfully:

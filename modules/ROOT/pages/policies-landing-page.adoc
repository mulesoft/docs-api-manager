...
+
You can apply a policy to any API implemented with Mulesoft as long as the API is exposed through an HTTP listener. You can also apply a policy to APIs not implemented with Mulesoft by deploying a Proxy application to control how and when a received request is forwarded to its implementation endpoint. API Manager supports the following proxy types: RAML/Rest, HTTP, or SOAP-based.

When applying a policy, you can set an API alert to notify you when an API request violates a policy for a given SLA. By default, a policy applies to the entire API, filtering traffic requests to every resource and method. This can be configured to provide resource-level granularity, if needed.

In Mule Runtime 3.8.0 and later, you can enhance security through policies by using Gatekeeper. Gatekeeper is a functionality that disables an API until all online policies are applied.

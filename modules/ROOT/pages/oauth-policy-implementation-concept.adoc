...
----
+
This token validation response may vary and, if *Expose Headers* is enabled, the proxy sends all headers received to the backend implementation with the exceptions of `scope` and `expires_in nodes`.

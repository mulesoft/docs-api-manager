...
As an owner of  a registered client application, you might be required to provide client application credentials when accessing an API. After a contract is created between the client application and the API, you can access the API using these credentials based on the policies that are applied. For example, if the API owner has applied the Client ID Enforcement policy on the API, you must provide the Client ID, and optionally, the Client Secret information from the client application. 

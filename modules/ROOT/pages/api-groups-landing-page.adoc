...
image:api-groups-architecture.png[API Group Hierarchy]

In this example, we have two business groups: `ORG A` and `ORG B`. Each of these business groups has two environments: `Sandbox` and `Production`. `ORG A` has two APIs: `Twitter` and `Facebook`, and one instance of each API in the two environments. `ORG B` has two APIs: `Instagram` and `Snapchat`, and one instance of each API in the two environments.

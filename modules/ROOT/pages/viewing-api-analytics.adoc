...
In this chart, the number of requests are not normalized for custom intervals. For example, monthly chart shows number of requests per 4 hours. For two months, the same chart shows requests per 8 hours. +
When using this in custom dashboards, you need to set explicit aggregation intervals.

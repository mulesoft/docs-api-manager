...
>s| Summary      | Transforms a tokenized value back into the original data. This policy is designed to work only for Anypoint Runtime Fabric deployments.
>s| Category | Security.
...
Detokenization is the process of detokenizing a token back into its original value to redeem sensitive data for fulfilling specific business needs. For example, an application may need to use a detokenized value to validate a specific credit card number. *<Eva, can we add a better example?>*
...
* One instance of the policy can be used to tokenize a value either in the request or the response, but not both.
...
The policy detokenizes certain pieces of the payload with the tokenized values. To define which pieces of payload will be detokenized, you must provide a selector expression.
...
The following flow of events occur when detokenizing requests:

image::policy-mule4-tokenization-67b25.png[]

. A user sends a request to the application protected by the policy.
...
. The detokenization policy is configured to detokenize the request. All the tokens are extracted from the payload using the selector expression.
...
The following flow of events occur when detokenizing responses:

image::policy-mule4-tokenization-9347d.png[]

. A user sends a request to the application protected by the policy.
. The request is first caught by Anypoint Security and redirected to the application, reaching first the detokenization policy.
. The detokenization policy is configured to detokenize the response of the Mule application. Therefore, it redirects the request without extracting anything from the payload.
. The Mule application processes the request and returns the result.
...
| No. +
If you do not specify a format, the policy will be applied. However, nothing is detokenized from your payload.

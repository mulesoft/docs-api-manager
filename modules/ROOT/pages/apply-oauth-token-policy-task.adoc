...
+
. Specify scopes that match one or more of the scopes defined in the referenced OAuth 2.0 Provider application.
. Starting with version 1.4.0, use the *Scope Validation Criteria* field to determine whether one or all of the scopes defined in the previous field must be present on the response from the identity provider.
+
...
Check this field to enable the propagation of properties from the identity provider to the backend. The properties are propagated as headers with the prefix `X-AGW-`.

image::expose-headers.png[height=100,width=770]

Propagated properties do not include the `scope`, `expires_in`, objects and arrays.

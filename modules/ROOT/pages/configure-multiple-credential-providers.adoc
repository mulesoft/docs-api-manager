...
In previous versions of Anypoint Platform, only a single IdP: either an external IdP or the default IdP (Anypoint Platform native client provider) was supported. However, with the multiple client providers feature, Anypoint Platform supports either one default IdP or multiple external IdPs.
...
* If previously did not assign a client provider to an environment when promoting and importing an API, after you assign an external client provider, all existing APIs in that environment still use the native Anypoint Platform client provider. 
+
...
+
You can user either the default IdP or multiple external IdPs. You cannot combine using an external client provider with the default provider.

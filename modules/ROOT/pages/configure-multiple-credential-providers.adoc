...
To promote an API from one environment to another, either both environments must use the same client provider or you must delete the target environment's IdP policy to prevent a policy mismatch. 

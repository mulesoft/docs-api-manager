...
When you promote an API to another environment, the target environment must have the client provider configured as in the source environment. Otherwise, a promoted API's IdP policy might not match the API's associated IdP. To resolve this mismatch, delete the IDP policy. 

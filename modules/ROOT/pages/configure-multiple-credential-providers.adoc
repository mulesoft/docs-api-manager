...
In previous versions of Anypoint Platform, only a single IdP: either an external IdP or the default IdP (Anypoint Platform native client provider) was supported. However, with the multiple client providers feature, Anypoint Platform supports either one default IdP, or one or more external IdPs.
...
* If you previously did not assign an external client provider to an environment when promoting and importing an API, the native provider is used by default. 
+
You can then reassign the appropriate external client provider for that API.
* If you assign an external client provider to an environment that was previously using a native provider, all existing APIs in that environment still use the native Anypoint Platform client provider. The new external IdP is used only by new APIs.
...
+
You can user either the default IdP, or one or more external IdPs. You cannot combine using an external client provider with the default provider.

...
In API Manager 2.2.13 and earlier, only a single identity provider (IdP) was supported: either an external IdP or the default IdP (Anypoint Platform native client provider). With the addition of the multiple client providers feature, API Manager 2.2.14 and later supports either one default IdP or one or more external IdPs.
...
To secure your APIs, create one external client provider per environment. Assign the existing client provider to your production environment and add the new providers to other environments, such as sandbox, and QA. However, check the possible impact on managed APIs and their consumers. 

Avoid using the same IdP in production and non-production environments. However, the same IdP can be used in multiple production environments, or in multiple non-production environments.
...
** Has contracts.
** Has client provider-related policies applied, such as PingFederate and OpenID policies.
...
+
You can use either the default IdP or one or more external IdPs. You cannot use the default provider and an external client provider together. To change an API's IdP from an external provider to an Anypoint Platform client provider, remove every IdP from the environment in which the API belongs.
...
| When promoting or importing an API between environments, specify an IdP from the target environment. | API Owner | API Manager | If there is just one (default) IdP specified for the target environment, it is automatically assigned to the API. 
...
When you promote an API to another environment, the target environment must have the client provider configured as in the source environment. Otherwise, a promoted API's IdP policy may not match the API's associated IdP. To resolve this mismatch, delete the IDP policy. 
...
+
As an Anypoint Administrator, you configure a client provider and assign it to specific environments.
. From *API Manager*, xref::create-instance-task.adoc[create or modify] APIs using the new IdPs.
+
As an API Owner create an API, import an API from Anypoint Exchange or modify an API in API Manager. Optionally, you can promote an API from one environment to another, and import or export an API, as described in the user flow diagram in the previous section. 

...
* An API created in an environment with an assigned external client provider will always use the external client provider, not the native Anypoint Platform client provider. The following describes one workaround:
+

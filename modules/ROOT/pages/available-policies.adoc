...
image::select-policy.png["List of available policies with categories for Compliance, Security, and Quality of Service"]
...
image::policy-filters.png["Policy filter interface showing category selection and protection level requirements"]
...
xref:http-basic-authentication-policy.adoc[HTTP Basic Authentication Policy]:: Enforces authentication per the details configured in a Security Manager policy.
xref:ip-blacklist.adoc[IP Blacklist]:: Denies API calls from a defined set of IP addresses.
xref:ip-whitelist.adoc[IP Allowlist]:: Limits API calls to a defined set of IP addresses.

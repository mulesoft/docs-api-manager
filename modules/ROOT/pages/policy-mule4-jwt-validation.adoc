...
>s| Category | Security
>s| Available from Mule version | 4.1.0
...
JSON Web Token (JWT) is a URL-secure method of representing claims to be transferred between two parties. The claims in a JWT are encoded as a JSON object that is used as the payload of a JSON Web Signature (JWS), or as the plaintext of a JSON Web Encryption (JWE) structure. This enables the claims to be digitally signed and integrity protected with a Message Authentication Code (MAC). Because the token is signed, you can trust the information and its source. 

The JWT Validation policy verifies the signature of the token and asserts the values of the claims of all incoming requests by using a JWT with JWS format. The policy does not validate JWT that uses JSON Web Encryption (JWE).

*Eva--are there any prerequisites and scope and considerations to document?*


== How This Policy Works

The policy verifies the signature of the JWT based on the values specified in the policy configuration. If the algorithm specified in the policy configuration does not match, or if the signature of the token is invalid all JWTs are rejected by the policy. If no algorithm is specified, the policy matches every signed and unsigned token.
...
Claim validations enables you to choose the conditions under which a token received in the policy is be rejected. The following registered claim validations are provided by default:
...
*Eva--these parameters in the policy UI have changed. Can you please verify which one is accurate please?*
...
|
| Token Expression | If you set the JWT Origin to *Custom Expression*, you should type the DataWeave expression returning the JWT here.
...
The policy uses public rsa key in pem format. 
After you obtain the public.pem key, remove the *-----BEGIN PUBLIC KEY-----* header and the *-----END PUBLIC KEY-----* footer before you paste it to the policy configuration.

*Eva--why must the customer perform these steps and when should they do this?*

...
== JWT Validation for Service Mesh

The Basic JWT Validation Policy works in the same way as the regular policy, with the following differences:

* Does not accept DataWeave expressions for claim validations
* Token retrieving and client ID validation parameters, which now are:

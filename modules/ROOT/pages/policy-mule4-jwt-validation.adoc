...
== Service Mesh Basic Version

For Service Mesh, there is a variety of this policy, named Basic JWT Validation Policy. Although the functionality is similar to the non-basic kind, there are differences in the policy configuration:
...
If you set it to `jwt` Header, the JWT will be extracted from that header. +
If you set it to HTTP Bearer Authentication Header, the JWT will be expected as Bearer. +
If you set this field to *Custom Header*, the name of the header where the JWT is expected must be provided.
|

| Token Header 
| If you set the JWT Origin to *Custom Header*, type the name of the header containing the JWT here.
| `myJwtHeader`
...
| Client ID Claim
| If Skip Client Id Validation is not set, the client ID needs to be extracted from the token.
| By default, the value will be extracted from the `client_id` claim.
...
| Validate Custom Claim
| Enables the usage of custom validations in the policy. The JWT will be valid only if all key-values match the expectation.
| The policy provides a _claimSet_ variable that contains all the claims present in the incoming JWT. For example:

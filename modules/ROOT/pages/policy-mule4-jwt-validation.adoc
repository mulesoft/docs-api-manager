...
After the policy parses and validates the JWT (use full form of JWT), the claims are propagated to downstream policies and flow, by using the *claims* property in the authentication object.
To access claim values, use the following Dataweave 2.0 expression: `#[authentication.properties.claims.<claimName>]`, where must be the name of the claim that you want to access.
...
The policy uses a public RSA key in .pem format. This section describes how to obtain the public .pem file that contains the key.
Before applying the content public.pem key to the *JWT Key* field in the policy configuration, ensure that you remove the following header and footer:

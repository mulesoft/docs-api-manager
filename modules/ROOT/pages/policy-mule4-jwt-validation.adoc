...
The JWT Validation policy validates the signature of the token and asserts the values of the claims of all incoming requests by using a JWT with JWS format. The policy does not validate JWT that uses JSON Web Encryption (JWE).


== How This Policy Works

The policy validates claims and signatures differently, as explained in the this section.

=== Validating Signature

The policy validates the signature of the JWT based on the values specified in the policy configuration. If the algorithm specified in the policy configuration does not match, or if the signature of the token is invalid, all JWTs are rejected by the policy. If no algorithm is specified, the policy matches every signed and unsigned token.
...
*No they haven't they are still the same, some of them are shown or not depending on the configured values for the other fields.*
...
If you have a preexisting RSA key in an unsupported format that you want to use, you must first covert the key to a PEM public key. The JWT policy uses public rsa key in pem format. 

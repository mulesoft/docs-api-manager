...
If you set it to Basic Authentication Header, the JWT will be expected as Bearer. +
...
| *Expose Headers*
| In a proxy scenario, defines whether claims should be exposed as headers in the request to the backend. +
The headers that may be sent are the claim names with an 'X-AGW-' prefix.
| If a JWT containing the payload {“foo”: ”value”, "iat": 1516239022} is sent, the policy will adds headers:

X-AGW-foo: value +
X-AGW-iat: 1516239022

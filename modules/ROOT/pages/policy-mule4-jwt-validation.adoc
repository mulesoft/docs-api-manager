...
Note that all DataWeave expressions must return a boolean value or they will always fail and that the vars only gets propageted if you are adding a custom validation. You can still access the claims using `#[authentication.properties.claims]`
|===

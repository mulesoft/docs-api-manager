...
** *Single Upstream*: +
If you are using Flex version 1.4.x or earlier or only want to configure a single upstream service, specify the following configuration settings.
+
...
To learn how to configure multiple upstream services, see <<traffic-managemen>>.
. Click *Next*.
+
...
In the following diagram, different routes manage requests to a flight information databases and to a customer service application. Route one has two upstream services defined directing 70% of requests to a stable database and 30% of requests to a beta database.  
...
Each API instance supports up to 10 routes that can support up to 10 upstream services. You define what routes receive what requests by defining route rules and route order. Only one route per API instance is required. 

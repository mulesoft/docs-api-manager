...
** *Single Upstream* +
If you are deploying an API instance to Flex Gateway version 1.4.x or earlier or only want to configure a single upstream service, specify the following configuration settings:
+
...
Each API instance supports up to 10 routes and each route can support up to 10 upstream services. You define what routes receive what requests by defining route rules and route order. Only one route per API instance is required. 
...
Method::::
+
The *Method* rule specifies the types of request methods the route can service.
+
You can specify multiple methods for each route. Only requests that are one of the specified methods are directed to this route.
+
To define the supported methods:
+
...
Path::::
+
The *Path* rule specifies the request path the route can service.
+
You can only specify one “URI Template Regex" path for each route. Only requests with the specified path are sent to this route.
+
...
Host::::
+
The *Host* rule specifies the request host the route can service.
+
You can only specify one host URL for each route. Only requests made from the specified host are sent to this route
+
...
Header::::
+
The *Headers* rule defines what header are required to be present or not present for this route to service the request.
+
For this rule, you must specify the header and a `true` or `false` value of whether or not the header is present. Only requests that meet all the specified header requirements are sent to this route. Additional headers present in the request that are not specified in the rules are ignored. 
+
To define the *Headers* rule:
+
. If this is not your first header, click *Add header (optional)*.
. Enter the header name.
. Enter `true` or `false` depending on if the header must be present.
+
To delete a header:
+
. Click the *Trash Can* icon (image:trash-can-icon.png[2%,2%]).  
...
For example, in a configuration where route one has the GET method applied as a rule and route two has no route rules applied, all GET requests are sent to route one and all other requests are sent to route two. If the route was reversed and route one had no route rules, Flex Gateway would direct all requests to the route one.

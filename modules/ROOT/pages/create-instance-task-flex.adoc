...
** *Single Upstream* +
If you are deploying an API instance to Flex Gateway version 1.4.x or earlier or only want to configure a single upstream service, configure the following settings:
+
...
Since any upstream within a route could service any request, all upstreams within the same route must adhere to the same API contract.

You can add a upstream by clicking *Add Upstream (optional)*, and you can delete a upstream by clicking the *Trash Can* icon (image:trash-can-icon.png[2%,2%]). If only one upstream is defined, you cannot delete that upstream.
...
NOTE: Each route can only support one upstream TLS Context. Adding a TLS context adds that TLS context for all upstreams within the same route.

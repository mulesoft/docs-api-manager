...
Note: Deploying these proxies versions in Mule 4.2.x and 4.1.x is possible, but it is not recommended since resource level policies work in compatibility mode in those Mule versions. This may force that the URI Template configurations have to be updated before migrating to Mule 4.3.x.

...
>s|Summary      | Logs custom messages using information from incoming requests, the backend, or information from other policies applied to the same API endpoint.
...
The Message Logging policy logs custom messages using information from incoming requests, the response from the backend, or information from other policies applied to the same API endpoint. 
...
`category`: This parameter value that typically indicates the package and class where the logging occurred. The default value is `org.mule`.
`message`: This parameter displays the details of the type of issue, for example, "the payload".
+
Example:
...
Note: If the payload is used as part of the message log, do not configure the Listener as non-repeatable (by default this setting is off).|Required
|Conditional |A DataWeave Boolean expression used to specify whether a message is to be logged. |Optional.
|Category |A prefix value in the log message that indicates the package and class where the log is executed. If no category is set, the default value is `org.mule.runtime.logging.policy-<policy id>`. |Optional

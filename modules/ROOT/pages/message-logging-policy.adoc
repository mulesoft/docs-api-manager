...
To better understand how to use the Message Logging policy, consider an example "Les Vetments" company that is in the business of retail clothing and more. To run their e-commerce portal successfully, Les Vetments needs to audit every event and transaction made on their website.
...
Les Vetments uses the resource-level policy's granularity to apply the policy to only `POST`, `PUT`, `PATCH` and `DELETE` methods. To log all events, Les Vetments applies the Message Logging policy to the API with the following UI configurations:
...
Now, whenever a customer accesses the Les Vetments catalogue and adds or removes items from their cart, or when an item is added or removed from the catalogue, logs are generated:
//add the output example here

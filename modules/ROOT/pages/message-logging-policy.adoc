...
>s|Summary      | Logs messages user-password defined on LDAP
>s|Category | Security
>s|First Mule version available | 4.0.0
.1+>.^s| Returned Status Codes
|401 - Unauthorized or invalid client application credentials
// what is the right ret code for this policy
|===

This policy logs a custom message with the information available between policies, proxy, or backend at any point of the execution. This policy works with both Apache log4j and Apache log4j2.

//Nahuel--what is meant by information available between policies, proxy, or backend at any point of the execution
...
`level`: The logging level or type of the message.

...
Policies implement an additional level of granularity. Policies control access based on the following access structuring:

* Resource-level policies
Applies the policy to only those request that match the criteria.  All policies, except the Cross-Origin Resource Sharing (CORS) policy, are resource-level.
* SLA-based policies

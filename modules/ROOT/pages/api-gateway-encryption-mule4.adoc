...
Gateway startup encryption allows you to encrypt sensitive information stored by the Mule runtime engine (Mule), such as policies, contracts, and the required credentials to configure the connection between your Mule application and Anypoint Platform.

IMPORTANT: This feature is only available for Mule versions 4.2.0 and later.

== Encrypting Your Anypoint Platform Credentials Using the Agent

Configuring encryption using the Runtime Manager Agent is the fastest way to secure your client credentials. You must add the encryption key when creating an Agent server. Subsequently, the agent handles the properties encryption for you. +
...
The most common way to ecrypt the properties is using the xref:mule-runtime::secure-configuration-properties.adoc#secure-properties-tool[Secure Properties Tools]. +
...
<4> And finally, specify theencryption key-value pair that you want to encrypt.
// end::example[]
...
+
<<<<<<< HEAD
For more information, see xref:mule-runtime::mule-app-properties-system.adoc#set-properties[Set System Properties when Starting Mule]
...
In the example, your `client_id` is generated as:
>>>>>>> v2.x
...
If your credentials cannot be decrypted, you cannot establish a connection with Anypoint Platform. Subsequently you will not be able to download policies or contracts.
...
|===
+
If this property is not set in the `wrapper.conf` file, the default `SENSITIVE_ONLY` encryption mode is used. +
...
[CAUTION]
--
If you are changing from `FULL` to `SENSITIVE_ONLY`, any parameter not marked as `sensitive` is be shown as plain text in your filesystem.
--

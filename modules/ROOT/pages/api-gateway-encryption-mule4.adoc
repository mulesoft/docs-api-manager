...
Gateway startup encryption allows you to encrypt sensitive information required to configure the connection between your Mule application and Anypoint Platform.
...
While the runtime is starting it checks for an encryption key. Then it gets the client ID and client secret values from its `wrapper.conf` file. If some of the values in that file are enclosed within the encryption indicators (`![` and `]`), the runtime uses the encryption key it loaded earlier to decrypt the values and use its original content.

This process can also be used to encrypt and decrypt sensitive values in your API policies.

You can use the Runtime Manager agent to encrypt your platform's credentials, or encrypt them and add them to your runtime's configuration manually.
...
This is important, as the runtime only uses this configuration to decrypt the value later.
...
[NOTE]
The `![` prefix and  `]` suffixes are required, as they indicate the runtime that the enclosed value is encrypted.

For the runtime to be able to decrypt your secured values, you need to configure your secret key using the `anypoint.platform.encryption_key` property in the wrapper file:
...
The runtime uses the encryption key configured in the `anypoint.platform.encryption_key` property to decrypt the properties.
...
By default, the values in a custom policy are not sensitive. You can define whether a value is sensitive or not in the YAML file describing your policy you can define which values are sensitive using the boolean element `sensitive`:
+

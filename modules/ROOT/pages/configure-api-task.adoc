...
| *Scheme* | Specifies whether to use HTTP or HTTPS. | Yes | If you select HTTPS, specify a TLS context for inbound traffic, outbound traffic, or both.
...
| **Advanced Options** |  |  |
| *TLS Context for outbound traffic* | Specifies the TLS Context used for the outbound traffic of HTTPS APIs. | No | xref:gateway::flex-conn-tls-config.adoc[Configure a TLS Context for Flex Gateway] before adding a TLS Context to your API.
|===

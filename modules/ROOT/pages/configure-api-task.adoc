...
. If you are creating the endpoint with proxy, in the *Implementation URI* field, either specify the URL of the site that hosts the API.
+
...
. If you selected the *Endpoint with proxy* option previously in this procedure, from the *Advanced Options*, optionally assign an instance label for the proxy, or accept the label, and set a response timeout.
. Optionally, at the top of *API Configuration* section, click *ADD A TAG* to enter a tag name, for example: Released, to signal the status of an API version to users.
. Click *Save*.

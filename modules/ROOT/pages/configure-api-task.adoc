...
+
Use this option to directly interact with a Mule runtime Engine (Mule) application configured for autodiscovery. 
Use this option to interact by proxy with a Mule application that cannot be automatically discovered, either because it is not configured for autodiscovery or because it uses a technology different from that of the API. In this case, the proxy must be both automatically discoverable and able to correctly forward application requests to the API. This is known as backend implementation.
+
* Proxy API endpoint configuration
+
Use this option to interact by proxy with a Mule application that cannot be automatically discovered, either because it is not configured for autodiscovery or because it uses a technology different from that of the API. In this case, the proxy must be both automatically discoverable and able to correctly forward application requests to the API. This is known as _backend implementation_.
...
.. In the *Application type* section, specify whether your application uses a Hybrid or CloudHub environment, or if you are running a non-Mule application.
...
+
<1> The application type: CloudHub, Hybrid, or non-Mule
...
.. In the Path field, specify the path.
//Path to what? and what is the TLS context? will our users know this?
.. Click *Advanced options* to specify additional settings.
... From the *Proxy version* list, choose the proxy version that you are currently using.
... In the *Validations* field, check the box if you want to validate inbound requests against a specification. You can optionally specify to use query parameters for strict validations.
... In the *Scheme* field, specify whether you want to use an HTTP or an HTTPs schema.
... In the *Port* field, specify the port number to use for the requests.
+
`8081` is the default port number. You can modify this port number, if required.
... Optionally, in the *Response timeout* field, specify the time (in seconds), when elapsed, the response must time out.
... Optionally, from the *Client provider* field, select the identity provider (IdP) that you want to use to authenticate access to this API from registered client applications.
+

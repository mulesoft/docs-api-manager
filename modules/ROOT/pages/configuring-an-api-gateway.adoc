...
For information about migrating from TLS 1.0, see xref:3.9@mule-runtime::tls1-0-migration.adoc[Migrate from TLS 1.0].
...
If you set up the Gateway with a client ID and client secret that belong to the main organization, these credentials work for all of the APIs in the organization, including APIs under a business group.
...
Before you configure your production gateway, you may want to configure Anypoint Studio to work with your Anypoint Platform organization for testing. First, xref:6.x@studio::install-studio-gw.adoc[download and install API Gateway Runtime] from the Studio Update Site.

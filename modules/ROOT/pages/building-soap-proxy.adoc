...
== Retrieving a WSDL using Basic Authentication credentials

The SOAP proxy provides a mechanism for retrieving a WSDL hosted on a remote site protected with Basic Auth. This is achieved by configuring the application properties `basic.auth.user` and `basic.auth.password`.

These properties are set in the `basicauth.properties` config file and passed to the validator module.
...
If no value is provided for the properties, then the WSDL retrieval is done without Basic Authentication.

== Retrieving a WSDL using TLS

In order to retrieve a WSDL using a TLS context, then the validator module present in the proxy must be configured using the `requesterConfig` parameter.

The value provided in the `requesterConfig` attribute must reference a valid `request-config` containing the TLS context to be used.

[source,xml,linenums]
----
<soap-request-validator:config name="validator-config" wsdlLocation="http://myremoteurl:9092/service?wsdl" serviceName="myServiceName"
                                   servicePort="myServicePort" schemaValidationEnabled="true" requesterConfig="https-request-config"/>

<http:request-config name="https-request-config" enableCookies="false" >
    <http:request-connection protocol="HTTPS" tlsContext="outbound-tls-context"/>
</http:request-config>

<tls:context name="outbound-tls-context">
     <tls:trust-store path="path/to/keystore.jks" password="keystorePassword" />
</tls:context>
----

If no value is provided for the `requesterConfig`, then the proxy will attempt retrieval with the default JVM configurations, provided TLS is needed.

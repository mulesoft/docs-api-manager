...
As from version 2.1.0, the SOAP Proxy supports as an out of the box feature retrieving a WSDL from a remote location reusing the TLS context configuration set in API Manager. The TLS context used will be the one configured for the outbound configuration of the proxy.

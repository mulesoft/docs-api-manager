...
You can retrieve a WSDL, hosted on a remote site, protected with a basic authentication by using the SOAP proxy mechanism. You configure the `basic.auth.user` and `basic.auth.password` application properties to retrieve the WSDL. These properties are set in the `basicauth.properties` config file and are passed to the validator module:
...
If no authentication values are provided for the properties, the WSDL is retrieved without the basic authentication.

== Retrieving a WSDL Using TLS

With version 2.1.0, the SOAP proxy is now provided as an out-of-the-box feature to retrieve a WSDL from a remote location by reusing the TLS context configuration that is set in API Manager. The TLS context configured for the outbound configuration of the proxy is used for the operation.

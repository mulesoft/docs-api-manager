...
API Manager 2.x enables you to use SOAP API proxies for your SOAP-based web services. The SOAP proxy requires a WSDL definition that describes the backend service. The  WSDL definition enables the API proxy to perform validations on incoming requests and correctly routes outbound requests to the corresponding API endpoint. 

Based on your configuration, validations are performed either against the SOAP message schema and the envelope, or only the envelope. By default, SOAP message schema validations are disabled.

== How schemas are Validated on SOAP Proxies

When you enable validations for an API, the SOAP action in the inbound request is matched against the same-named operation defined in the WSDL definition. After the matching operation is obtained, the proxy validates whether the namespace URI of the element and the name associated with the operation matches the same details in the incoming request. 
...
. In API Manager, select  *Manage API > Manage API from Exchange* to create your API Instance.
+
...
. Specify the API version and the asset version details in the *API version* and *Asset version* fields for the instance that you are creating.
. From the *Asset type* field, select `SOAP API - WSDL`. 
+
. In the *File upload* field, upload the corresponding WSDL file.
+
...
. To configure your API proxy for this API instance, from the *Managing Type* field, select the *Endpoint with Proxy* option.
. Select your Proxy Deployment Target from the available deployment targets displayed.
. If you are managing this API in Mule 4 or above, select the *Mule version* checkbox. 
. Specify appropriate details in the *Implementation URI*, *Service Name*, *Service Port* and *Service Namespace* fields. 
...
.. Optionally, specify an label and response timeout period in the *API Instance label* and a *Response Timeout* fields.
. Click *Save*.

== Troubleshooting SOAP Proxy for Missing Imports on XSD Files

A SOAP proxy can fail in deploy time due to an incorrect WSDL definition. For example, the following error indicates missing imports in one of the XSD files that are a part of the WSDL: 

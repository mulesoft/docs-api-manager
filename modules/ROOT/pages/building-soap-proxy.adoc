...
Regardless of whether validation is enabled for an API, the SOAP request validator always verifies that the following are true:  

* The incoming request has a valid SOAP action
* The payload is a valid SOAP envelope

If validation is enabled for an API, the SOAP request validator also verifies that the content of `body` tag matches the one defined in WSDL. The SOAP action in the inbound request is matched against the same-named operation in the WSDL. The proxy then validates that the namespace URI of the element and the name associated with the operation matches the same details in the incoming request. 
...
.. Select the *Validations* checkbox if you want the content of `body` tag to be validated on incoming requests in addition to the default validations as described in <<SOAP Proxy Schema Validation>>.

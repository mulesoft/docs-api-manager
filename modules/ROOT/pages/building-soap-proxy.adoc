...
== Retrieving a WSDL by Using Basic Authentication Credentials

You can retrieve a protected WSDL that is hosted on a remote site by using the SOAP proxy mechanism to configure the `basic.auth.user` and `basic.auth.password` application properties in the `basicauth.properties` configuration file. The properties are then passed to the validator module:
...
With SOAP proxy v2.1.0, the proxy is configured out-of-the-box to retrieve a WSDL from a remote location. The TLS context configured for the outbound configuration of the proxy is used to retrieve the WSDL.

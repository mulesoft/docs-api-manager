...
Depending on the OAuth grant type you want to use, the provider might expose two or three endpoints. You can configure these endpoint paths in the mule properties file of the provider. The custom link:/api-manager/v/1.x/v/1.x/building-an-external-oauth-2.0-provider-application[OAuth 2 provider example] configures mule properties in the `config.properties` file as follows:
...
After building the provider, you use the validation endpoint path again when you link:/api-manager/v/1.x/v/1.x/external-oauth-2.0-token-validation-policy#applying-the-oauth-2-0-token-validation-policy[apply the OAuth 2.0 policy] to the API.
...
When you link:/api-manager/v/1.x/v/1.x/external-oauth-2.0-token-validation-policy#applying-the-oauth-2-0-token-validation-policy[apply the policy] to an API, you enter a list of supported OAuth scopes.

== Implementing CORS

To apply the CORS policy, see link:/api-manager/v/1.x/v/1.x/cors-policy[Applying and Editing a CORS Policy].

...
Maven is used to build and deploy Mule 4 policies. This means that policies consists of two files:
...
In order to package a policy in Maven, the development project must be composed of the following files

* An XML template
+
Contains the implementation of the policy using Mule's XML schema language.
* A YAML file
+
Defines the configurable parameters of the policy. Anypoint API Manager uses this file to render the UI to display inputs for the policy.
+
* The template's pom.xml file
+
Defines policy dependencies. The packaging type needs to be `mule-policy`. The pom.xml file may
define any other Maven plugin that helps to manage the development lifecycle of the Maven project.
* Resources
+
...
+
NOTE: Policies cannot export resources nor packages such as Java classes.
...
<2> The `http-policy:source` block contains the actual instructions to execute, relative to the Mule flow the policy is configured
to handle. The `http-policy:source` block can inject instructions before an HTTP Listener event fires at the start
...
<4> The `http-policy:execute-next` element is required to continue the Mule Event processing. The `http-policy:execute-next`
element can trigger other policies or the application flow. Any policy can stop the Mule Event processing chain by not
...
The policy with the lower order executes first. The flow will always be executed in the middle of the policy operations.
...
For example, by placing the `http-policy:execute-next` element inside a `choice` element, the next Mule Event execution can continue
or stop, depending on the choice condition.

In the below example, the flow is will only execute when the incoming request has a header named `myHeader` and
...
Next, the XML http-transform namespace must be added to expose operations by the above dependency.
...
NOTE: It is not possible for a policy to use the Java Module because the Java Module requires exported Java classes.
Policies are not allowed to export resources nor packages such as Java classes.

== Outbound policies

Policies can be applied on outbound HTTP Requests within a flow as well. This capability enables policies to inject
...
Once an error is caught by an `error-handler`, the error is either

1. Propagated up the Mule Event processing chain
2. Handled, where normal Mule event process execution continues
...
Since the flow’s error-handler is configured to not propagate errors, `<FEH>` will return to the policy and `<P2>`
...
From the above example, the flow never executes.

**Scenario 3**

Assume now that the flow's error handler propagates errors now.
...
The error gets propagated back to the policy and the policy's error handler continues to handle the error.

In summary,
 error handlers provide an alternative method to conditionally execute a flow within a policy.
...
Classloading isolation between policies and Mule 4 applications are not completely isolated. This scenario is
...
You can read more on classloading isolation here: https://docs.mulesoft.com/mule4-user-guide/v/4.1/about-classloading-isolation
...
Policies can make changes to the Mule message, but the changes may not get propagated, depending
on where the Mule message changes are made relative to the `http-policy:execute-next` element.
...
made **before** the `execute-next` element are always propagated. Modifications made after the `execute-next` element,
...
Dependent on where the Mule message is set within a policy block, the attribute `propagateMessageTransformations` may need
to be set to true to enable Mule message propagation.
...
The Authentication object can be accessed through DataWeave
...
The only method to set the Authentication object is through a dependency using the SDK Mule extension. Refer to this
link on how to set the Authentication object: https://docs.mulesoft.com/mule-sdk/v/1.1/authentication-handler

Mule 4 out-of-the-box policies in Anypoint API Manager use this exact method to propagate Client information.
...
may want to allow up to 100 request per minutes on one API, and allow 5000 per minute on a different API.
...
<1> List of options for the user to choose. Each option has a name for displaying in the UI and an internal value that used in the policy.
...
implementing semantic logic, such as conditionals. This is the preferred method of using the user inputs in the policies.
...
NOTE: Handlebars is an extension of Mustache, which was used in earlier versions of Mule policies.
...
== Pointcuts
In earlier versions, the pointcut element was required to configure a custom policy. Pointcuts specified which API the policy operates.
In Mule 4, there is no need to configure the pointcut element. Information, on which API the custom policy operations on,
is provided by the Anypoint API Manager when a policy is applied.

...
You can deploy your API proxy to CloudHub, Runtime Fabric, or a hybrid cloud environment. By default, the resources allocated to the proxy application include a CPU with 0.1 vCores and a memory of 500 MB. If required, modify the resource allocation from xref:runtime-manager::deploying-to-cloudhub#change-settings-of-a-deployed-application.adoc[Runtime Manager].

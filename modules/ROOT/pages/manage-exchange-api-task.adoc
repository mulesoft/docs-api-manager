...
+
The parameters that you must specify differs based on the Mule version you select. For detailed field descriptions, see <<field-reference,parameter details>>.
...
|===
. Optionally, to fine tune additional configurations for the API, expand *Advanced options* and specify the <<field-reference,details>>, and then click *Save*.

[[field-reference]]
== Field Input Reference

The input that you are required to provide when adding an API from Exchange differs on several factors, such as whether you want to create an endpoint with proxy, and whether the API was created using Mule 3 or Mule 4:
...
| *Enable Console* | Specifies whether you can simulate your API specification. | No | You can specify a different path in *Console Path*, for example, "/spec/*".
...
| *API instance label* | Specifies a label for the API. | No | If you have multiple managed instances of the same API, add a label to differentiate each instance from the another.
...
// * In *Port*, specify a different port if the displayed port is incorrect.
// * In *Response timeout*, specify a response timeout, if required. 
// * If you choose *Hybrid* as the proxy deployment target, select *Reference user domain* to use an API gateway domain.
// +
...
// * If you want to validate inbound requests against a provided specification, select *Validations* and specify whether you want query parameters validated. 

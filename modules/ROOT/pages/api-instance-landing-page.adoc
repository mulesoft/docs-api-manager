...
+
API Manager generates the apiId of new APIs managed by API Manager for use with Mule 4.
...
+
While transitioning consumers of your API to an updated instance, you can prevent developers from signing up for access to your old API instance. 
** xref:delete-api-task.adoc[Delete an API Instance]
+
You can delete an API instance and its version if you no longer need to manage it from API Manager. 

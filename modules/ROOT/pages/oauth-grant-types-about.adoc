...
OAuth 2.0 provides the following grant types methods for requesting for a token:
...
For the RAML-based APIs in which the RAML must be updated with the OAuth 2.0 security schema, the following table maps the RAML grant types to the grant type names in the OAuth 2.0 policy configuration:

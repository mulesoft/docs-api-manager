...
From API Manager, you can apply policies to APIs in Flex Gateway Connected Mode, Mule Gateway, or Anypoint Service Mesh.
...
For gRPC APIs, API Manager displays only policies that are enabled for gRPC and supported by Flex Gateway.

NOTE: gRPC policies apply only to gRPC APIs and are supported only on Flex Gateway.
